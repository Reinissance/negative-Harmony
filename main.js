    ...
    function onMIDIFailure(msg) {
      ...
      midiIn.innerHTML = "No extern MIDI available.";
    ...
    function setupMidiPlayer() {
        ...
        if (!playing) {
          if (midiData) {
            ...
            Tone.Transport.scheduleRepeat(() => {
              if (playing) {
                const progress = (Tone.Transport.seconds / midiData.duration) * 100;
      ...
      progressSlider.oninput = function (event) {
        const position = (event.target.value / 100) * midiData.duration;
    ...
    function setupGMPlayer() {
      ...
      window.handleNoteOnForChannel = function (note, velocity, channel) {
        ...
        if (channel === 9) {
          ...
          const randomVelocityAddition = Math.random() * 0.1; - 0.05;
          velocity = Math.min(1, Math.max(0, velocity + randomVelocityAddition));
    ...
    function createControlsForChannel(channel, programNumber, sfIndex, name) {
        ...
        instSelect.onchange = function (event) {
          ...
          availableInstrumentsForProgramChange[event.target.selectedIndex].preset = preset;
